...
import type { Lot, Produit, SortiePayload } from "../types";
import { btnPrimary, card, errorText, fieldError, input, label } from "../ui";

const emptyForm: SortiePayload = { lotId: "", quantite: "", motif: "" };

export default function StockSortie() {
    const navigate = useNavigate();
    const [produitId, setProduitId] = useState("");
    const [produits, setProduits] = useState<Produit[]>([]);
    const [lots, setLots] = useState<Lot[]>([]);
    const [form, setForm] = useState<SortiePayload>(emptyForm);
    ...
    return (
        <div className="mx-auto max-w-xl">
            <h1 className="mb-6 text-2xl font-semibold text-slate-900">Sortie de stock</h1>
            ...
            <div className={card}>
                ...
                <form onSubmit={handleSubmit} className="space-y-4">
                        ...
                        >
                            ...
                            {lots.map((l) => (
                                <option key={l.id} value={l.id}>
                                    {l.numeroLot ?? "Sans numéro"} (Stock: {l.quantite})
                                </option>
                            ))}
                        </select>
                    ...
                    <div>
                        <label className={label}>Quantité à sortir</label>
                        <input
                            type="number"
                            step="0.01"
                            min="0.01"
